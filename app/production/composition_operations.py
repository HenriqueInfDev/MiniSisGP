...
def validate_bom_item(product_id, material_id):
    ...
    material = conn.execute('SELECT DESCRICAO, TIPO_ITEM FROM T_ITEM WHERE ID = ?', (material_id,)).fetchone()
...
def get_bom(product_id):
    ...
    bom = conn.execute('''
        SELECT
            ...
            I.CUSTO_MEDIO
        FROM T_COMPOSICAO C
        JOIN T_ITEM I ON C.ID_INSUMO = I.ID
...
def add_bom_item(product_id, material_id, quantity):
    ...
    try:
        conn = get_db_manager().get_connection()
        conn.execute(
            'INSERT INTO T_COMPOSICAO (ID_PRODUTO, ID_INSUMO, QUANTIDADE) VALUES (?, ?, ?)',
...
def update_bom_item(bom_id, quantity):
    ...
    conn = get_db_manager().get_connection()
    conn.execute(
        'UPDATE T_COMPOSICAO SET QUANTIDADE = ? WHERE ID = ?',
        (quantity, bom_id)
    )
    conn.commit()
...
def delete_bom_item(bom_id):
    ...
    conn = get_db_manager().get_connection()
    conn.execute('DELETE FROM T_COMPOSICAO WHERE ID = ?', (bom_id,))
    conn.commit()
...
def update_composition(product_id, new_composition):
    ...
    try:
        with conn:
            cursor.execute("DELETE FROM T_COMPOSICAO WHERE ID_PRODUTO = ?", (product_id,))
            if new_composition:
                cursor.executemany(
                    "INSERT INTO T_COMPOSICAO (ID_PRODUTO, ID_INSUMO, QUANTIDADE) VALUES (?, ?, ?)",
