...
def create_op(due_date, items_to_produce):
    ...
    try:
        ...
        op_id = cursor.execute(
            "INSERT INTO T_ORDEM_PRODUCAO (DATA_CRIACAO, DATA_PREVISTA, STATUS) VALUES (?, ?, 'Planejada')",
            (current_date, due_date)
        ).lastrowid
        for item in items_to_produce:
            cursor.execute(
                "INSERT INTO T_ORDEM_PRODUCAO_ITENS (ID_ORDEM_PRODUCAO, ID_PRODUTO, QUANTIDADE_PRODUZIR) VALUES (?, ?, ?)",
...
def update_op(op_id, due_date, items_to_produce):
    ...
    cursor = conn.cursor()
    try:
        cursor.execute("UPDATE T_ORDEM_PRODUCAO SET DATA_PREVISTA = ? WHERE ID = ?", (due_date, op_id))
        cursor.execute("DELETE FROM T_ORDEM_PRODUCAO_ITENS WHERE ID_ORDEM_PRODUCAO = ?", (op_id,))
        for item in items_to_produce:
            cursor.execute(
                "INSERT INTO T_ORDEM_PRODUCAO_ITENS (ID_ORDEM_PRODUCAO, ID_PRODUTO, QUANTIDADE_PRODUZIR) VALUES (?, ?, ?)",
                (op_id, item['id_produto'], item['quantidade'])
...
def get_op_details(op_id):
    conn = get_db_manager().get_connection()
    op_master = conn.execute("SELECT * FROM T_ORDEM_PRODUCAO WHERE ID = ?", (op_id,)).fetchone()
    if not op_master:
        return None
    op_items = conn.execute("""
        SELECT OPI.ID_PRODUTO, I.DESCRICAO, OPI.QUANTIDADE_PRODUZIR, U.SIGLA AS UNIDADE
        FROM T_ORDEM_PRODUCAO_ITENS OPI
        JOIN T_ITEM I ON OPI.ID_PRODUTO = I.ID
...
def list_ops(search_term="", search_field="id"):
    conn = get_db_manager().get_connection()
    query = "SELECT ID, DATA_CRIACAO, DATA_PREVISTA, STATUS FROM T_ORDEM_PRODUCAO"
    params = ()
    if search_term:
