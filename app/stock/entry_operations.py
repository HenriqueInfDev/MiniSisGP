...
def create_entry(entry_date, supplier, note_number):
    ...
    try:
        cursor.execute(
            "INSERT INTO ENTRADANOTA (DATA_ENTRADA, FORNECEDOR, NUMERO_NOTA, STATUS) VALUES (?, ?, ?, 'Em Aberto')",
...
def update_entry_master(entry_id, entry_date, supplier, note_number):
    ...
    try:
        conn.execute(
            "UPDATE ENTRADANOTA SET DATA_ENTRADA = ?, FORNECEDOR = ?, NUMERO_NOTA = ? WHERE ID = ?",
...
def update_entry_total_value(entry_id, total_value):
    """Atualiza o valor total de uma nota de entrada."""
    conn = get_db_connection()
    try:
        ...
        conn.rollback()
        print(f"Erro ao atualizar valor total da entrada: {e}")
        return False
...
def update_entry_items(entry_id, items):
    ...
    try:
        ...
        update_entry_total_value(entry_id, total_value)
        return True
    except sqlite3.Error as e:
...
def get_entry_details(entry_id):
    ...
    items = conn.execute("""
        ...
        JOIN ITEM ti ON tei.ID_INSUMO = ti.ID
        JOIN UNIDADE tu ON ti.ID_UNIDADE = tu.ID
...
def list_entries(search_term="", search_field="id"):
    ...
    conn = get_db_connection()
    query = "SELECT ID, DATA_ENTRADA, FORNECEDOR, NUMERO_NOTA, VALOR_TOTAL, STATUS FROM ENTRADANOTA"
    params = ()
        ...
        with conn:
            ...
            for item in details['items']:
                ...
                current_item = cursor.execute(
                    "SELECT SALDO_ESTOQUE, CUSTO_MEDIO FROM ITEM WHERE ID = ?",
                    (insumo_id,)
                ).fetchone()
            ...
            cursor.execute(
                "UPDATE ENTRADANOTA SET VALOR_TOTAL = ?, STATUS = 'Finalizada' WHERE ID = ?",
